// Package moonrisemoonset provides a Go client for the Moonrise Moonset API.
//
// For more information, visit: https://moonrisemoonset.apiverve.com?utm_source=go&utm_medium=readme
package moonrisemoonset
...
// Request contains the parameters for the Moonrise Moonset API.
//
// Parameters:
//   - lat (required): number - The latitude of the location [min: -90, max: 90]
//   - lon (required): number - The longitude of the location [min: -180, max: 180]
//   - date: string - The date to get the moonrise and moonset times for (MM-dd-yyyy) [format: date]
type Request struct {
...
func (r *Request) Validate() error {
	rules := map[string]ValidationRule{
		"lat": {Type: "number", Required: true, Min: float64Ptr(-90), Max: float64Ptr(90)},
		"lon": {Type: "number", Required: true, Min: float64Ptr(-180), Max: float64Ptr(180)},
...
// ResponseData contains the data returned by the Moonrise Moonset API.
type ResponseData struct {
	Coordinates CoordinatesData `json:"coordinates"`
	Phase string `json:"phase"`
	Moonrise string `json:"moonrise"`
	Moonset string `json:"moonset"`
	MoonAlwaysUp bool `json:"moonAlwaysUp"`
	MoonAlwaysDown bool `json:"moonAlwaysDown"`
}

// CoordinatesData represents the coordinates object.
